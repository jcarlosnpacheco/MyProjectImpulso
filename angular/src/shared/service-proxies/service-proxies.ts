...
export class CandidateDtoPagedResultDto implements ICandidateDtoPagedResultDto {
  items: CandidateDto[] | undefined;
  totalCount: number;

  constructor(data?: ICandidateDtoPagedResultDto) {
    if (data) {
      for (var property in data) {
        if (data.hasOwnProperty(property))
          (<any>this)[property] = (<any>data)[property];
  ...
  static fromJS(data: any): ICandidateDtoPagedResultDto {
    data = typeof data === "object" ? data : {};
    let result = new CandidateDtoPagedResultDto();
    result.init(data);
    return result;
  }
  ...
  clone(): ICandidateDtoPagedResultDto {
    const json = this.toJSON();
    let result = new CandidateDtoPagedResultDto();
    result.init(json);
    return result;
...
export interface ICandidateDtoPagedResultDto {
  items: CandidateDto[] | undefined;
  totalCount: number;
}
...
@Injectable()
export class CandidateServiceProxy {
  private http: HttpClient;
  private baseUrl: string;
  protected jsonParseReviver: ((key: string, value: any) => any) | undefined =
    undefined;
  ...
  getAll(
    skipCount: number | undefined,
    maxResultCount: number | undefined
  ): Observable<CandidateDtoPagedResultDto> {
    let url_ = this.baseUrl + "/api/services/app/Candidate/GetAll?";
        ...
        _observableCatch((response_: any) => {
          if (response_ instanceof HttpResponseBase) {
            try {
              ...
              return <Observable<CandidateDtoPagedResultDto>>(
                (<any>_observableThrow(e))
              );
            }
          } else
  ...
  protected processGetAll(
        ...
        _observableMergeMap((_responseText) => {
          ...
          let resultData200 =
            _responseText === ""
              ? null
              : JSON.parse(_responseText, this.jsonParseReviver);
          result200 = CandidateDtoPagedResultDto.fromJS(resultData200);
          return _observableOf(result200);
        })
      );
    } else if (status !== 200 && status !== 204) {
    ...
    return _observableOf<CandidateDtoPagedResultDto>(<any>null);
  }
}
